...
async function loadData() {
  // Returns { status, data }
  // status: "ok" (real data), "empty" (genuinely just the init placeholder), "error" (load failed — DO NOT overwrite)
  try {
    ...
    if (decoded && Object.keys(decoded).length > 0 && !decoded.init) {
      return { status: "ok", data: decoded };
    }
    // Genuinely empty (init placeholder)
...
// ---- Static data ----
const DEFAULT_TEAMS = [
  { id:"tA", name:"Tree to Green",    color:"#16a34a" },
  { id:"tB", name:"Josh Does Sports", color:"#2563eb" },
  { id:"tC", name:"BackNine Boys",    color:"#dc2626" },
];
...
const DEFAULT_SCHEDULE = [
  ...
  { number:3, type:"standard", foursomes:[
    ...
    { teamA:"tA", teamB:"tC", front:["pCole","pKaden","pTanner","pBraden"],        back:[["pCole","pTanner"],    ["pKaden","pBraden"]]  },
    { teamA:"tB", teamB:"tC", front:["pJosh","pAustin","pCaden","pAlex"],           back:[["pJosh","pAlex"],      ["pAustin","pCaden"]] },
  ]},
...
function front9Info(format, t1p, t2p) {
  ...
  if (format === "Shamble") {
    // Each player gets (handicap * 0.75 * 0.5) strokes vs the lowest handicap in the foursome
    const minH = Math.min(...h1,...h2);
...
// ---- Standings ----
function data_winnerPlayers(round, teamId) {
  // For scramble: all registered players on a given team
  return (round._allPlayers||[]).filter(p=>p.teamId===teamId).map(p=>p.id);
}
function computeStandings(teams, players, rounds) {
  ...
  rounds.forEach(round => {
    ...
    if (round.type==="scramble5") {
      round._allPlayers = players;
      // New scramble scoring: 1st=3, 2nd=1, 3rd=0, CTP=0.5, Long Drive=0.5
      if (round.firstTeam)  { team(round.firstTeam, 3);  data_winnerPlayers(round, round.firstTeam).forEach(p=>mvp(p,0.75,"2v2")); }
      if (round.secondTeam) { team(round.secondTeam, 1); data_winnerPlayers(round, round.secondTeam).forEach(p=>mvp(p,0.25,"2v2")); }
      if (round.ctpPlayer)  { const pl=players.find(p=>p.id===round.ctpPlayer); if(pl){ team(pl.teamId,0.5); mvp(round.ctpPlayer,0.5,"ctp"); } }
      if (round.ldPlayer)   { const pl=players.find(p=>p.id===round.ldPlayer);  if(pl){ team(pl.teamId,0.5); mvp(round.ldPlayer,0.5,"ctp"); } }
    } else {
...
export default function App() {
  ...
  useEffect(() => {
    loadData().then(result => {
      if (result.status === "ok") {
        ...
        // Only seed defaults when the store is genuinely empty (first-ever run)
        const rounds = [{
        ...
        setData(init);
        saveData(init, setSaveStatus);
      } else {
        // status === "error" — load failed. DO NOT seed or overwrite. Show error, allow retry.
        setLoadError(true);
      }
      setLoading(false);
    });
  ...
  if (loadError || !data) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"sans-serif",color:"#374151",flexDirection:"column",gap:16,padding:20,textAlign:"center"}}>
      <div style={{fontSize:32}}>⚠️</div>
      <div style={{fontWeight:600}}>Couldn't load league data</div>
      <div style={{fontSize:13,color:"#6b7280",maxWidth:320}}>There was a problem reaching the server. Your data is safe — this is just a loading issue. Please refresh to try again.</div>
      <button onClick={()=>window.location.reload()} style={{padding:"10px 20px",borderRadius:8,border:"none",cursor:"pointer",fontSize:14,fontWeight:600,background:"#16a34a",color:"#fff"}}>Refresh</button>
    </div>
  );
              ...
              <div style={{fontSize:13}}>
                {round.completed?(
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {round.firstTeam&&<div>🥇 <b style={{color:teamById[round.firstTeam]?.color}}>{teamById[round.firstTeam]?.name}</b> (+3 pts)</div>}
                    {round.secondTeam&&<div>🥈 <b style={{color:teamById[round.secondTeam]?.color}}>{teamById[round.secondTeam]?.name}</b> (+1 pt)</div>}
                    {round.thirdTeam&&<div>🥉 {teamById[round.thirdTeam]?.name}</div>}
                    {round.ctpPlayer&&<div>📍 CTP: <b>{playerById[round.ctpPlayer]?.name}</b> (+0.5)</div>}
                    {round.ldPlayer&&<div>🏌️ Long drive: <b>{playerById[round.ldPlayer]?.name}</b> (+0.5)</div>}
                  </div>
  ...
  const RoundEditor = ({round, onClose}) => {
    ...
    const handleSave=completed=>{
      ...
      // Rebuild matches from the (possibly edited) schedule so Results & scoring stay in sync.
      // Preserve entered results only where the matchup players are unchanged.
      let rebuiltMatches = r.matches;
      if (r.type !== "scramble5") {
        const old = r.matches || [];
        const rebuilt = [];
        let oldIdx = 0;
        schedDraft.forEach(fg => {
          const all = fg.front.map(id=>playerById[id]).filter(Boolean);
          const t1 = all.filter(p=>p.teamId===fg.teamA).map(p=>p.id);
          const t2 = all.filter(p=>p.teamId===fg.teamB).map(p=>p.id);
          // front9
          const oldFront = old[oldIdx];
          const frontSame = oldFront && oldFront.type==="front9"
            && JSON.stringify([...(oldFront.team1Players||[])].sort())===JSON.stringify([...t1].sort())
            && JSON.stringify([...(oldFront.team2Players||[])].sort())===JSON.stringify([...t2].sort());
          rebuilt.push(frontSame ? oldFront : {type:"front9",format:oldFront?.format||"Scramble",team1:fg.teamA,team2:fg.teamB,team1Players:t1,team2Players:t2,result:"",winTeam:"",winPlayers:[]});
          oldIdx++;
          // back9s
          fg.back.forEach(([p1,p2])=>{
            const oldBack = old[oldIdx];
            const backSame = oldBack && oldBack.type==="back9" && oldBack.player1===p1 && oldBack.player2===p2;
            const pp1=playerById[p1],pp2=playerById[p2];
            rebuilt.push(backSame ? oldBack : {type:"back9",player1:p1,player2:p2,team1:pp1?.teamId||"",team2:pp2?.teamId||"",result:"",winPlayer:"",winTeam:""});
            oldIdx++;
          });
        });
        // keep CTP (last item) if present
        const oldCtp = old.find(m=>m.type==="ctp");
        rebuilt.push(oldCtp || {type:"ctp",winPlayer:"",winTeam:""});
        rebuiltMatches = rebuilt;
      }
      const updated={...r,matches:rebuiltMatches,subs,completed};
        ...
        <div style={{background:"#fff",borderRadius:14,padding:18,width:"100%",maxWidth:560,alignSelf:"flex-start"}}>
          ...
          {r.type==="scramble5"?(
            <div style={{marginBottom:12}}>
              <div style={{fontWeight:600,fontSize:13,marginBottom:10}}>🏆 Scramble Results</div>
              <div style={{...s.row,marginBottom:8}}>
                <label style={{fontSize:13,width:90,flexShrink:0}}>1st place</label>
                <select style={s.inp} value={r.firstTeam||""} onChange={e=>setR({...r,firstTeam:e.target.value})}><option value="">Select</option>{tOpts}</select>
              </div>
              <div style={{...s.row,marginBottom:8}}>
                <label style={{fontSize:13,width:90,flexShrink:0}}>2nd place</label>
                <select style={s.inp} value={r.secondTeam||""} onChange={e=>setR({...r,secondTeam:e.target.value})}><option value="">Select</option>{tOpts}</select>
              </div>
              <div style={{...s.row,marginBottom:8}}>
                <label style={{fontSize:13,width:90,flexShrink:0}}>3rd place</label>
                <select style={s.inp} value={r.thirdTeam||""} onChange={e=>setR({...r,thirdTeam:e.target.value})}><option value="">Select</option>{tOpts}</select>
              </div>
              <div style={{...s.row,marginBottom:8}}>
                <label style={{fontSize:13,width:90,flexShrink:0}}>📍 Closest pin</label>
                <select style={s.inp} value={r.ctpPlayer||""} onChange={e=>setR({...r,ctpPlayer:e.target.value})}><option value="">Select player</option>{data.players.map(p=><option key={p.id} value={p.id}>{p.name} ({teamById[p.teamId]?.name})</option>)}</select>
              </div>
              <div style={s.row}>
                <label style={{fontSize:13,width:90,flexShrink:0}}>🏌️ Long drive</label>
                <select style={s.inp} value={r.ldPlayer||""} onChange={e=>setR({...r,ldPlayer:e.target.value})}><option value="">Select player</option>{data.players.map(p=><option key={p.id} value={p.id}>{p.name} ({teamById[p.teamId]?.name})</option>)}</select>
              </div>
            </div>
          ):(r.matches||[]).map((m,i)=>(
